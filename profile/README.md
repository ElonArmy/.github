...
- 그러나 위의 손실함수는 단순히 경계를 늘리는 것에대한 손실함수이고 현실의 문서가 복원되는 정확한 정보에 대해서는 부정확하다. 따라서 a cycle-consistent segmentation mask loss라는 손실함수를 사용해야한다 $$L_{M} = |\phi(y,d_{E})-m|_1 + |\phi(m,d\^{-1}\_{E} - y)|_1$$
...
- 인조의 데이터만을 학습 시키는 것은 어느 수준을 넘어가면 정확도가 향상되지 않는다. 따라서 현실의 데이터가 효과적으로 도메인갭을 보완해준다
- Enet은 doc3d(일부)데이터셋으로 손실L_se을 사용해해 먼저 학습하고 doc3d(나머지 데이터)와diw(현실) 데이터셋을 합친것을 가지고 혼합손실함수 L_se + (0.1)L_m 를 사용하여 파인튜닝된것이다.
