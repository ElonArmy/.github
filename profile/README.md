...
### 1/27
  ...
  - 찬란 ⇒ 논문 발전과정 조사하면서 핵심 구조등 기초 기술(코드) 수집, rectinet 사전학습 모델을 사용해서 테스트 해보기, 성능과 결과물의 상태
  - 지섭님 => docunet 모델 , 데이터생성해서 학습 시키고 책페이지에 대한 결과물 테스트 해보기 docunet깃허브에 상세히 설명되어있음, 성능과 결과물의 상태
  - json =>  flutter 여쭤보기,
  - oh님 => 리액트네이티브 여쭤보기
  - 경훈님 => docunet 모델 구조를 참고하여 FDRnet 구현해보기
  - molar님 => 책페이지 이미지가 들어왔을때  페이지를 포착하고, 정형화된(방향, 위치, 크기 등) 표현으로 바꿔주는STNs 구현해보기(깃헙 심플디워프)
